...
- Headless stats, logs, sweeps, tuning summaries, and seed-bank reports now expose total chemical A/B field mass so signaling is visible during tuning
- Headless stats, logs, sweeps, tuning summaries, and seed-bank reports now expose total nutrient and detritus pools so stored environmental energy is visible during tuning
- Headless stats, logs, sweeps, tuning summaries, and seed-bank reports now expose population-level recent speed so locomotion is visible during headless tuning
- Headless stats, logs, sweeps, tuning summaries, and seed-bank reports now expose active and peak grip-latch counts so contact-heavy predator behavior is visible during headless tuning
- Headless stats, logs, sweeps, tuning summaries, and seed-bank reports now expose current seasonal light intensity and direction so light-regime shifts remain visible during headless tuning
- Headless stats, logs, sweeps, tuning summaries, and seed-bank reports now expose mean gripper contact and active signals so predator engagement quality is visible during headless tuning
- Headless stats, logs, sweeps, tuning summaries, and seed-bank reports now expose mean and max creature ages so lifecycle durability is visible during headless tuning
- Headless stats, logs, sweeps, tuning summaries, and seed-bank reports now expose mean and max creature energy so individual survivor quality is visible during headless tuning
...
- Both debug viewer backends now support selectable nutrient, light, chemical A/B, and detritus overlays for fuller local field inspection
...
- The shipped demo seed set now uses a spec-aligned four-node worm grazer plus larger tripod/triangle archetypes, and both debug viewers now draw clearer mouth/gripper/sensor/photoreceptor glyphs
