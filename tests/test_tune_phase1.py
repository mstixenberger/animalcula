...
def test_tune_phase1_writes_raw_and_summary_outputs(tmp_path: Path) -> None:
    ...
    assert "avg_population_variance" in summaries[0]
    assert "peak_population_max" in summaries[0]
    assert "avg_population_capacity_fraction" in summaries[0]
    assert "peak_population_capacity_fraction_max" in summaries[0]
    assert "avg_crowding_multiplier" in summaries[0]
    ...
    assert "avg_mean_motor_edges_per_creature" in summaries[0]
    ...
    assert "avg_mean_photoreceptors_per_creature" in summaries[0]
    assert "avg_nutrient_total" in summaries[0]
