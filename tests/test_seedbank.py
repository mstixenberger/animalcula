...
def test_evaluate_seed_bank_ranks_candidates_by_aggregate_results(tmp_path: Path) -> None:
    ...
    assert "avg_mean_edges_per_creature" in report["rankings"][0]
    assert "avg_mean_motor_edges_per_creature" in report["rankings"][0]
    assert "avg_mean_segment_length_per_creature" in report["rankings"][0]
    assert "avg_mean_mouths_per_creature" in report["rankings"][0]
    assert "avg_mean_grippers_per_creature" in report["rankings"][0]
    assert "avg_mean_sensors_per_creature" in report["rankings"][0]
    assert "avg_mean_photoreceptors_per_creature" in report["rankings"][0]
    ...
    assert "avg_mean_age_ticks" in report["rankings"][0]
    assert "max_age_ticks_max" in report["rankings"][0]
